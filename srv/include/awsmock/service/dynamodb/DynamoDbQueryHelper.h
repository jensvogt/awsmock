      ...
      private:
        ...
        std::string parse_operator() {
            ...
            if (c == '>' || c == '<') {
                ...
                log_error << "Expected '=' after '" << std::to_string(c) << "' at position " << std::to_string(pos_);
                throw Core::DynamoDbParseException(std::string("Expected '=' after '") + c + "' at position " + std::to_string(pos_));
            }
            ...
            return std::visit([&]<typename T0>(T0 &&c) -> value {
                ...
                if constexpr (std::is_same_v<T, Eq>) {
                    if (Dto::DynamoDb::AttributeValue value = resolve(c.placeholder, attr_values); value.type == "S") {
                        ...
                        return make_document(kvp(getField(c.field, value.type), value.numberValue));
                    }
                }
