...
namespace Awsmock::Service {
    ...
    CognitoServer::CognitoServer(Core::Scheduler &scheduler) : AbstractServer("cognito"), _module("cognito"), _scheduler(scheduler) {
        ...
        _monitoringPeriod = Core::Configuration::instance().get<int>("awsmock.modules.cognito.monitoring-period");
        _backupActive = Core::Configuration::instance().get<bool>("awsmock.modules.cognito.backup.active");
        _backupCron = Core::Configuration::instance().get<std::string>("awsmock.modules.cognito.backup.cron");
    ...
    void CognitoServer::UpdateCounter() const {
        ...
        for (auto &userPool: _cognitoDatabase.listUserPools({}, {}, 0, 0, {})) {
