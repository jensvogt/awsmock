...
namespace AwsMock::Database {
    ...
    bool SNSDatabase::TopicExists(const std::string &topicArn) const {
        ...
        if (HasDatabase()) {
            ...
            try {
                mongocxx::collection topicCollection = GetCollection(_topicCollectionName);

                const int64_t count = topicCollection.count_documents(make_document(kvp("topicArn", topicArn)));
    ...
    bool SNSDatabase::TopicExists(const std::string &region, const std::string &topicName) const {
        ...
        if (HasDatabase()) {
            ...
            try {
                ...
                const int64_t count = topicCollection.count_documents(make_document(kvp("region", region), kvp("topicName", topicName)));
                log_trace << "Topic exists: " << std::boolalpha << count;
                return count > 0;
            } catch (const mongocxx::exception &exc) {
                log_error << "SNS Database exception " << exc.what();
    ...
    Entity::SNS::Topic SNSDatabase::CreateTopic(Entity::SNS::Topic &topic) const {
        ...
        if (HasDatabase()) {
            ...
            mongocxx::collection topicCollection;
            auto session = GetSession(_topicCollectionName, topicCollection);

            try {

                session.start_transaction();
                const auto result = topicCollection.insert_one(topic.ToDocument());
    ...
    Entity::SNS::Topic SNSDatabase::GetTopicById(bsoncxx::oid oid) const {
        ...
        try {
            ...
            if (const auto mResult = topicCollection.find_one(make_document(kvp("_id", oid)))) {
    ...
    Entity::SNS::Topic SNSDatabase::GetTopicByArn(const std::string &topicArn) const {
        ...
        if (HasDatabase()) {
            ...
            try {
                ...
                if (const auto mResult = topicCollection.find_one(make_document(kvp("topicArn", topicArn))); !mResult->empty()) {
                    Entity::SNS::Topic result;
                    result.FromDocument(mResult->view());
                    return result;
                }
    ...
    Entity::SNS::Topic SNSDatabase::GetTopicByTargetArn(const std::string &targetArn) const {
        ...
        if (HasDatabase()) {
            ...
            try {
                ...
                if (const auto mResult = topicCollection.find_one(
    ...
    Entity::SNS::TopicList SNSDatabase::GetTopicsBySubscriptionArn(const std::string &subscriptionArn) const {
        ...
        if (HasDatabase()) {
            ...
            try {
                ...
                for (auto queueCursor = topicCollection.find(make_document(kvp("subscriptions.subscriptionArn", subscriptionArn))); const auto topic: queueCursor) {
                    Entity::SNS::Topic result;
        ...
        return _memoryDb.GetTopicsBySubscriptionArn(subscriptionArn);
    }
    ...
    Entity::SNS::TopicList SNSDatabase::ListTopics(const std::string &region) const {
        if (HasDatabase()) {
            try {
                ...
                Entity::SNS::TopicList topicList;
                for (auto queueCursor = topicCollection.find(query.extract()); const auto topic: queueCursor) {
                    Entity::SNS::Topic result;
                    result.FromDocument(topic);
        ...
        return _memoryDb.ListTopics(region);
    }
    ...
    Entity::SNS::TopicList SNSDatabase::ListTopics(const std::string &prefix, const long pageSize, const long pageIndex, const std::vector<SortColumn> &sortColumns, const std::string &region) const {
        ...
        if (HasDatabase()) {
            try {
                ...
                mongocxx::collection _queueCollection = GetCollection(_topicCollectionName);
    ...
    Entity::SNS::Topic SNSDatabase::UpdateTopic(Entity::SNS::Topic &topic) const {
        ...
        if (HasDatabase()) {
            ...
            try {
                ...
                const auto mResult = topicCollection.find_one_and_update(make_document(kvp("region", topic.region), kvp("topicArn", topic.topicArn)), topic.ToDocument(), opts);
    ...
    void SNSDatabase::UpdateTopicCounter(const std::string &topicArn, const long messages, const long size, const long initial, const long send, const long resend) const {
        ...
        if (HasDatabase()) {
            ...
            try {
                ...
                topicCollection.update_one(filterQuery.extract(), updateQuery.extract());
    ...
    long SNSDatabase::CountTopics(const std::string &region, const std::string &prefix) const {
        ...
        if (HasDatabase()) {
            ...
            try {
                ...
                const long count = topicCollection.count_documents(query.extract());
    ...
    long SNSDatabase::PurgeTopic(const Entity::SNS::Topic &topic) const {
        ...
        if (HasDatabase()) {
            ...
            mongocxx::collection topicCollection;
            auto session = GetSession(_messageCollectionName, topicCollection);

            try {
                session.start_transaction();
                const auto result = topicCollection.delete_many(make_document(kvp("topicArn", topic.topicArn)));
    ...
    long SNSDatabase::GetTopicSize(const std::string &topicArn) const {
        ...
        if (HasDatabase()) {
            ...
            mongocxx::collection _objectCollection = GetCollection(_messageCollectionName);
    ...
    long SNSDatabase::DeleteAllTopics() const {
        ...
        if (HasDatabase()) {
            ...
            try {
                session.start_transaction();
                const auto result = topicCollection.delete_many({});
                session.commit_transaction();
    ...
    bool SNSDatabase::MessageExists(const std::string &messageId) const {
        ...
        if (HasDatabase()) {
            ...
            try {
                mongocxx::collection messageCollection = GetCollection(_messageCollectionName);

                const int64_t count = messageCollection.count_documents(make_document(kvp("messageId", messageId)));
    ...
    Entity::SNS::Message SNSDatabase::CreateMessage(Entity::SNS::Message &message) const {
        ...
        if (HasDatabase()) {
            ...
            mongocxx::collection messageCollection;
            auto session = GetSession(_messageCollectionName, messageCollection);
    ...
    long SNSDatabase::CountMessages(const std::string &topicArn) const {
        ...
        if (HasDatabase()) {
            ...
            try {
                ...
                const long count = messageCollection.count_documents(query.extract());
    ...
    long SNSDatabase::CountMessagesByStatus(const std::string &topicArn, const Entity::SNS::MessageStatus status) const {
        ...
        if (HasDatabase()) {
            ...
            try {
                ...
                const long count = messageCollection.count_documents(make_document(kvp("topicArn", topicArn), kvp("status", MessageStatusToString(status))));
    ...
    void SNSDatabase::DeleteMessage(const std::string &messageId) const {
        ...
        if (HasDatabase()) {
            ...
            try {
                session.start_transaction();
                const auto result = messageCollection.delete_one(make_document(kvp("messageId", messageId)));
    ...
    void SNSDatabase::DeleteMessages(const std::string &region, const std::string &topicArn, const std::vector<std::string> &messageIds) const {
        ...
        if (HasDatabase()) {
            ...
            try {
                ...
                session.start_transaction();
                const auto result = messageCollection.delete_many(make_document(kvp("region", region), kvp("topicArn", topicArn), kvp("messageId", make_document(kvp("$in", array)))));
