...
#define CRONCPP_CONSTEXPTR
    ...
    namespace utils {
        ...
        inline std::string to_upper(std::string text) {
            std::ranges::transform(text, std::begin(text), [](char const c) { return static_cast<char>(std::toupper(c)); });
    ...
    namespace detail {
        ...
        static std::pair<cron_int, cron_int> make_range(CRONCPP_STRING_VIEW field, cron_int const minval, cron_int const maxval) {
            cron_int first = 0;
            cron_int last = 0;
        ...
        template<size_t N>
        static void set_cron_field(CRONCPP_STRING_VIEW value, std::bitset<N> &target, cron_int const minval, cron_int const maxval) {
            if (!value.empty() && value[value.length() - 1] == ',')
                throw bad_cronexpr("Value cannot end with comma");

            const auto fields = utils::split(value, ',');
            if (fields.empty())
            ...
            for (auto const &field: fields) {
                if (!utils::contains(field, '/')) {
                    ...
                    const auto delta = detail::to_cron_int(parts[1]);
                    if (delta <= 0)
        ...
        template<size_t N>
        size_t next_set_bit(std::bitset<N> const &target, size_t, size_t, const size_t offset) {
    ...
    static std::tm cron_next(cronexpr const &cex, std::tm date) {
        const time_t original = utils::tm_to_time(date);
