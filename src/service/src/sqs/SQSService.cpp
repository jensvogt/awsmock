...
namespace AwsMock::Service {
    Dto::SQS::CreateQueueResponse SQSService::CreateQueue(const Dto::SQS::CreateQueueRequest &request) const {
        ...
        if (_sqsDatabase.QueueArnExists(queueArn)) {
            ...
            return {
                .region = queue.region, .name = queue.name, .owner = queue.owner, .queueUrl = queue.queueUrl,
        ...
        try {
            ...
            Database::Entity::SQS::Queue queue = {
                .region = request.region, .name = request.queueName, .owner = request.owner, .queueUrl = queueUrl,
                .queueArn = queueArn, .attributes = attributes, .tags = request.tags
            };
        ...
        try {
            if (request.maxResults > 0) {
                ...
                Dto::SQS::ListQueuesResponse listQueueResponse = {
                    .queueList = queueList, .nextToken = nextToken, .total = total
                };
    ...
    Dto::SQS::ListQueueAttributeCountersResponse SQSService::ListQueueAttributeCounters(
        ...
        try {
            ...
            response.attributeCounters.emplace_back(attributeCounter);
            attributeCounter = {
                .attributeKey = "delaySeconds", .attributeValue = std::to_string(queue.attributes.delaySeconds)
            };
            response.attributeCounters.emplace_back(attributeCounter);
            attributeCounter = {
                .attributeKey = "maxMessageSize", .attributeValue = std::to_string(queue.attributes.maxMessageSize)
            };
            response.attributeCounters.emplace_back(attributeCounter);
            attributeCounter = {
            ...
            response.attributeCounters.emplace_back(attributeCounter);
            attributeCounter = {
                .attributeKey = "redriveAllowPolicy", .attributeValue = queue.attributes.redriveAllowPolicy
            };
            response.attributeCounters.emplace_back(attributeCounter);
            attributeCounter = {
    ...
    Dto::SQS::ListLambdaTriggerCountersResponse SQSService::ListLambdaTriggerCounters(
        ...
        try {
            std::vector<Database::Entity::Lambda::Lambda> lambdas = Database::LambdaDatabase::instance().
                    ListLambdasWithEventSource(request.queueArn);
            ...
            for (const auto &lambda : lambdas) {
                if (lambda.HasEventSource(request.queueArn)) {
                    ...
                    Dto::SQS::LambdaTriggerCounter triggerCounter = {
                        .uuid = eventSourceMapping.uuid, .arn = lambda.arn, .enabled = eventSourceMapping.enabled
                    };
 ...
 " queueUrl: " << queueUrl;
    ...
    void SQSService::SetVisibilityTimeout(Dto::SQS::ChangeMessageVisibilityRequest &request) const {
        ...
        try {
            ...
            const Database::Entity::SQS::MessageAttribute messageAttribute = {
                .attributeName = "VisibilityTimeout", .attributeValue = std::to_string(request.visibilityTimeout),
    ...
    Dto::SQS::SendMessageResponse SQSService::SendMessage(const Dto::SQS::SendMessageRequest &request) const {
        ...
        try {
            ...
            if (std::vector<Database::Entity::Lambda::Lambda> lambdas = Database::LambdaDatabase::instance().
                    ListLambdasWithEventSource(queueArn); !lambdas.empty()) {
            ...
            for (const auto &entry : request.entries) {
                ...
                try {
                    ...
                    Dto::SQS::MessageSuccessful s = {
                        .id = entry.id, .messageId = response.messageId, .md5Body = response.md5Body,
                        .md5UserAttr = response.md5UserAttr, .md5SystemAttr = response.md5SystemAttr
                    };
                    sqsResponse.successful.emplace_back(s);
                } catch (Core::DatabaseException &exc) {
                    Dto::SQS::MessageFailed f = {
                        .id = Core::StringUtils::CreateRandomUuid(), .message = exc.message(), .senderFault = false
                    };
    ...
    void SQSService::DeleteMessage(const Dto::SQS::DeleteMessageRequest &request) const {
        ...
        try {
            const Database::Entity::SQS::Message message = _sqsDatabase.
                    GetMessageByReceiptHandle(request.receiptHandle);
 ...
 ", deleted: " << deleted;
    ...
    void SQSService::SendLambdaInvocationRequest(const Database::Entity::Lambda::Lambda &lambda,
        ...
        Dto::SQS::Record record = {
            .region = lambda.region, .messageId = message.messageId, .receiptHandle = message.receiptHandle,
            .body = message.body, .attributes = message.attributes, .md5Sum = message.md5Body, .eventSource = "aws:sqs",
        ...
        log_debug << "Lambda send invocation request finished, function: " << lambda.function << " sourceArn: " <<
 eventSourceArn;
    }
} // namespace AwsMock::Service
