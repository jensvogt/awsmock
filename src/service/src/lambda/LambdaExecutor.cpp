...
namespace AwsMock::Service {
    ...
    std::string LambdaExecutor::Run(const Database::Entity::Lambda::Lambda &lambda, const std::string &containerId, const std::string &host, const int port, const std::string &payload, const std::string &functionName, const std::string &receiptHandle) const {
        ...
        if (response.statusCode != http::status::ok) {
            ...
            result.httpStatusCode = Core::HttpUtils::StatusCodeToString(response.statusCode);
            result.lambdaStatus = Database::Entity::Lambda::InstanceIdle;
            output = response.body.empty() ? "success" : response.body;
        }
