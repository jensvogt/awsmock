...
namespace Awsmock::Core {
    ...
    BOOST_AUTO_TEST_CASE(CronTimestampTest) {
        ...
        // act
        const system_clock::time_point datetime = CronUtils::GetNextExecutionTime("0 0 0 * * ?") + std::chrono::seconds(DateTimeUtils::UtcOffset());
    ...
    BOOST_AUTO_TEST_CASE(CronSecondsTest) {
        ...
        // act
        const long datetimeSeconds = CronUtils::GetNextExecutionTimeSeconds("0 0 0 * * ?") + DateTimeUtils::UtcOffset();
        const system_clock::time_point midnight = std::chrono::ceil<std::chrono::days>(system_clock::now());
