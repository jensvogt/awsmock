/*
 * This script provides a Chrome browser extension that allows to easily test Web applications against LocalStack. Once enabled, the extension
 * intercepts all API calls made to AWS services (*.amazonaws.com) and redirects the calls to AwsMock (running on http://localhost:4566 by default).
 */
    ...
    XMLHttpRequest.prototype.open = function (...args) {
        ...
        if (args.length > 2 && match) {
            ...
            if (match[1].match(/.*execute-api.*/)) {
                args[1] = `http://${match[1]}${AWSMOCK_HOST}/${path}`;
            }
            const request = _getRequest(this);
    ...
    XMLHttpRequest.prototype.send = function (...args) {
        ...
        if (request._proxyRequest) {
            ...
            if (!proxyRequest.open_sent) {
                ...
                proxyRequest.open_args[1] = `http://${AWSMOCK_HOST}${params.path || "/"}`;
...
const handleEventMessage = (event) => {
    ...
    const getListener = (request, eventName) => {
        ...
        return request[listenerAttrName];
    };
...
if (isPageContext) {
    console.log("Initializing LocalSurf extension, redirecting AWS service calls to AwsMock");
